...
//    av_register_all();
    ...
    std::cerr << "Format =  " << codecpar->format << std::endl;
  ...
  //   std::cout << "[info] Frame size: " << frame_size << " bytes (" << width << "x" << height << ", YUYV)\n";
    ...
    //    char* src_buf = (char*)malloc(480*640*get_bpp_from_format(RK_FORMAT_YVYU_422));
        uint8_t* dst_buf = (uint8_t*)malloc(640*640*get_bpp_from_format(RK_FORMAT_RGB_888));

        src_handle = importbuffer_virtualaddr(pkt->data, 640, 480, RK_FORMAT_YCbCr_420_P);
        dst_handle = importbuffer_virtualaddr(dst_buf, 640, 640, RK_FORMAT_RGB_888);
        src = wrapbuffer_handle(src_handle, 640, 480, RK_FORMAT_YCbCr_420_P);
