...
public:
    ProcessedQueue(): size(0){
        debug("Processed Queue created");
    }
    std::vector<std::vector<NamedBbox>> arr;

    void push(std::vector<NamedBbox> bboxes) {
        // std::lock_guard<std::mutex> lock(queue_mutex);
       std::lock_guard<std::mutex> lock(proc_mutex);
       arr.push_back(bboxes);
       size++;
    ...
    std::vector<NamedBbox>& read() {
        auto start = std::chrono::high_resolution_clock::now();
        std::unique_lock<std::mutex> lock(queue_mutex);
        if (RESIZED == 1) {
            draw_bounding_boxes(queue.read(), arr.front());
            debug("Showing image");
            cv::imshow("Inference", queue.read());
        }else {
            draw_bounding_boxes(queue.read_orig(), arr.front());
            debug("Showing image");
        ...
        post_times.push_back(duration.count() / 1000);
        queue.pop_front();
        lock.unlock();
        std::lock_guard<std::mutex> lock1(proc_mutex);
        pop();
        size--;
    }   
    
};
ProcessedQueue proc_queue;
...
//    printFirstBytes(output_buffer.get());
    ...
    proc_queue.push(  parse_nms_data(output_buffer.get(), 80)) ;
    // std::unique_lock<std::mutex> lock(queue_mutex);
    // if (RESIZED == 1) {
...
void show() {
    while (true) {
        ...
        if (STOP_CAP == 0 && proc_queue.size == 0) {
            lock.unlock();
            break;
        }else if (proc_queue.size == 0) {
            lock.unlock();
            continue;
        }
...
int main(int argc, char* argv[]) {
    ...
    std::thread preprocess(capture, &cap1);
    ...
    // sum2 += post_times[i];
    // }
    // std::cout << "Average inference time = " << sum / DELAY << std::endl;
    std::cout << "Average FPS = " << DELAY / (duration.count() / 1000)<< std::endl;
    // std::cout << "Average full FPS = " << DELAY / sum1 << std::endl;
